...
void MyCobotBasic::setup(HardwareSerial * serial)
{
    mycobot_serial = serial;
...
bool MyCobotBasic::asynOrSync()
{
    mycobot_serial->write(header);
    mycobot_serial->write(header);
    mycobot_serial->write(ASYN_OR_SYNC_LEN);
    mycobot_serial->write(ASYN_OR_SYNC);
    mycobot_serial->write(footer);

    unsigned long t_begin = millis();
    void *tempPtr = nullptr;
    int *pReturnVersion = nullptr;
    bool asyn_or_sync = false; 

    while (true) {
        if (millis() - t_begin > 40)
            break;
        if (readData(a_invalid, asyn_or_sync, r_invalid, e_invalid, m_invalid) == -2)
            continue;
        else {
            return asyn_or_sync;
        }
...
int MyCobotBasic::readData(T& pData, O& bData, R& rData, E& eData, M& mData)
{
    ...
    byte data_len[1];
    byte r_data_3[3], r_data_4[4], r_data_14[14], r_data_8[8], r_data_6[6];
    switch (static_cast<int>(data[0])) {
        case 3:
            ...
            switch (int(r_data_3[0])) {
                ...
                case GET_GRIPPER_MODE: {
                    bData = bool(r_data_3[1]);
                    return 1;
                }

                case IS_FREE_MOVE_MODE: {
                    bData = bool(r_data_3[1]);
                    return 1;
                }
        ...
        case 6:
            DivideSerialData(data, r_data_6, 6);
            switch (int(r_data_6[0])) {
                case GET_SERVO_LAST_PDI: {
                    //1-->servo id
                    pData.at(0) = r_data_6[2];
                    pData.at(1) = r_data_6[3];
                    pData.at(2) = r_data_6[4];
                    return 1;
                }
                default:
                    break;
...
void MyCobotBasic::writeCoords(Coords coord, int speed, byte mode)
{
...
void MyCobotBasic::clearServoQueue()
{
    mycobot_serial->write(header);
    mycobot_serial->write(header);
    mycobot_serial->write(CLEAR_SERVO_QUEUE_LEN);
...
Angles MyCobotBasic::getServoLastPDI(int joint)
{
    mycobot_serial->write(header);
    mycobot_serial->write(header);
    mycobot_serial->write(GET_SERVO_LAST_PDI_LEN);
    mycobot_serial->write(GET_SERVO_LAST_PDI);
    mycobot_serial->write(joint);
    mycobot_serial->write(footer);
    ...
    while (true) {
        ...
        if (readData(PDI, invalid, r_invalid, e_invalid, m_invalid) == -2) {
            continue;
        } else {
            //Serial.println("have data");
            //Serial.println(millis() - t_begin);
...
void MyCobotBasic::setEletricGripper(bool mode)
{
    mycobot_serial->write(header);
    mycobot_serial->write(header);
    mycobot_serial->write(SET_ELETRIC_GRIPPER_LEN);
    mycobot_serial->write(SET_ELETRIC_GRIPPER);
    mycobot_serial->write(mode);
    mycobot_serial->write(footer);
...
void MyCobotBasic::InitEletricGripper()
{
    mycobot_serial->write(header);
    mycobot_serial->write(header);
    mycobot_serial->write(INIT_ELETRIC_GRIPPER_LEN);
    mycobot_serial->write(INIT_ELETRIC_GRIPPER);
    mycobot_serial->write(footer);
...
void MyCobotBasic::setGripperMode(bool mode)
{
    mycobot_serial->write(header);
    mycobot_serial->write(header);
    mycobot_serial->write(SET_GRIPPER_MODE_LEN);
    mycobot_serial->write(SET_GRIPPER_MODE);
    mycobot_serial->write(mode);
    mycobot_serial->write(footer);
...
bool MyCobotBasic::getGripperMode()
{
    mycobot_serial->write(header);
    mycobot_serial->write(header);
    mycobot_serial->write(GET_GRIPPER_MODE_LEN);
    mycobot_serial->write(GET_GRIPPER_MODE);
    mycobot_serial->write(footer);
    ...
    while (true) {
        if (millis() - t_begin > 40)
            break;
        if (readData(a_invalid, mode, r_invalid, e_invalid, m_invalid) == -2)
            continue;
        else {
            return mode;
        }
...
void MyCobotBasic::setFreeMoveMode(bool mode)
{
    mycobot_serial->write(header);
    mycobot_serial->write(header);
    mycobot_serial->write(SET_FREE_MOVE_MODE_LEN);
    mycobot_serial->write(SET_FREE_MOVE_MODE);
    mycobot_serial->write(mode);
    mycobot_serial->write(footer);
...
bool MyCobotBasic::isFreeMoveMode()
{
    mycobot_serial->write(header);
    mycobot_serial->write(header);
    mycobot_serial->write(IS_FREE_MOVE_MODE_LEN);
    mycobot_serial->write(IS_FREE_MOVE_MODE);
    mycobot_serial->write(footer);
...
void MyCobotBasic:: releaseAllServos(bool mode)
{
    mycobot_serial->write(header);
    mycobot_serial->write(header);
    if (mode)
        mycobot_serial->write(RELEASE_ALL_SERVOS_LEN+1);
    else
        mycobot_serial->write(RELEASE_ALL_SERVOS_LEN);
    mycobot_serial->write(RELEASE_ALL_SERVOS);
    if (mode)
        mycobot_serial->write(mode);
    mycobot_serial->write(footer);
}
...
void MyCobotBasic::releaseServo(byte servo_no, bool mode)
{
    mycobot_serial->write(header);
    mycobot_serial->write(header);
    if (mode)
        mycobot_serial->write(RELEASE_SERVO_LEN+1);
    else
